...
当前最高巡航速度为 `cruise_speed = 18.0`。直道上使用更大的动态预瞄距离提升高速稳定性；弯道中通过路径曲率压缩预瞄距离，并继续使用横向加速度限制降低实际输出速度，当前 `max_lat_acc = 3.6`。为减少弯道入口慢半拍和左右摆动，控制器会把前方路径的带符号曲率作为转向前馈，并用路径曲率提前限制弯道速度。

## 前车跟车和超车

前方同向车通过自车坐标系下的 `x_rel`、`y_rel` 和航向差分类。若间距小于 `min_gap + ego_speed * time_headway` 或 TTC 小于阈值，进入 `FRONT_CAR_FOLLOW`，用安全间距误差调节速度。前车低速或停车时只继续跟车/停车等待，不再因为等待超过固定时间自动生成左右平滑偏移轨迹。

## 路口汇入让行

T 字路口等汇入场景使用未来参考路线冲突预测。控制器会按当前路线中心线向前预测 3.5s，并用邻车匀速直线模型检查未来路线点是否会在 3m 内接近邻车预测位置。若存在冲突，进入 `JUNCTION_YIELD`，按 `junction_yield_decel = 3.0` 减速；冲突时间小于 `emergency_ttc` 时直接停车。`ADJACENT_LANE_ONCOMING` 在平行相邻车道行驶时仍被忽略，但当自车未来左转路线会与其预测轨迹相交时必须让行。
